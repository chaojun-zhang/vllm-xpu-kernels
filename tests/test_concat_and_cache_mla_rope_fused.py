...
DEVICES = [
    f"xpu:{i}"
    for i in range(1 if torch.xpu.device_count() == 1 else 2)
]
...
def test_concat_and_cache_mla_rope_fused(
    ...
    # Use the PyTorch-native RoPE as reference.
    # NOTE: forward_native is executed first because the custom kernel is
    # in-place.
    ...
    assert total_available_slots >= total_needed_slots, \
        "Not enough kv slots!"
    ...
    kv_cache_scale = torch.tensor([0.1],
                                  dtype=torch.float32,
                                  device=device)
    ...
    # Build the reference KV cache by scattering kv_c + RoPE'd k_pe.
    ...
    for i in range(seq_len):
        ...
        ref_temp[block_idx, block_offset] = torch.cat(
            (kv_c[i], ref_k_rope[i]), -1)

    if kv_cache_dtype == "fp8":
        ref_kv_cache = torch.empty_like(ref_temp, dtype=kv_cache.dtype)
        convert_fp8(ref_kv_cache, ref_temp, kv_cache_scale.item(),
    ...
    if kv_cache_dtype == "fp8":
        ...
        convert_fp8(result_temp, kv_cache.contiguous(),
                    kv_cache_scale.item(), kv_dtype=kv_cache_dtype)
        expected_temp = torch.empty_like(ref_kv_cache, dtype=torch.float16)
        convert_fp8(expected_temp, ref_kv_cache,
                    kv_cache_scale.item(), kv_dtype=kv_cache_dtype)
        torch.testing.assert_close(result_temp, expected_temp,
                                   atol=0.001, rtol=0.1)
    else:
